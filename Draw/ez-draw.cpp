...
int ez_init(void)
{
    ...
    ezx.key_name = ezx.key_string = NULL;
...
Ez_window ez_window_create(int w, int h, const char *name, Ez_func func)
{
    ...
    if (ez_check_state("ez_window_create") < 0)
        return None;
    ...
    win = CreateWindowEx(
        ...
        h + 4 + GetSystemMetrics(SM_CYSIZEFRAME) * 2 + GetSystemMetrics(SM_CYCAPTION),
...
{
    if (ez_check_state("ez_window_show") < 0)
        return;
...
void ez_window_get_size(Ez_window win, int *w, int *h)
{
    ...
    w_ret = rect.right - rect.left - 4 - GetSystemMetrics(SM_CXSIZEFRAME) * 2;
    if (w_ret < 0)
        w_ret = 0;
    h_ret = rect.bottom - rect.top - 4 - GetSystemMetrics(SM_CYSIZEFRAME) * 2 - GetSystemMetrics(SM_CYCAPTION);
    if (h_ret < 0)
        h_ret = 0;
...
void ez_main_loop(void)
{
    ...
    {
        Ez_win_info *info;
        if (ez_info_get(ezx.win_l[i], &info) < 0)
            continue;
        if (info->show)
            ez_window_show(ezx.win_l[i], 1);
    }
...
void ez_HSV_to_RGB(float H, float S, float V, float* r, float* g, float* b)
{
    if(H>360 || H<0 || S>100 || S<0 || V>100 || V<0){
        cout<<"The HSV values are not in valid range"<<endl;
        exit(0);
    }
    float s = S/100;
    float v = V/100;
...
Ez_uint32 ez_get_HSV(float h, float s, float v)
{
    float r, g, b;
...
void ez_set_color(Ez_uint32 color)
{
    ...
    if (ezx.hbrush != NULL)
        DeleteObject(ezx.hbrush);
    ezx.hbrush = CreateSolidBrush(ezx.color);
    if (ezx.dc_win != None)
        SelectObject(ezx.hdc, ezx.hbrush);
...
void ez_draw_point(Ez_window win, int x1, int y1)
{
    ...
    if (ezx.thick == 1)
        ...
        XFillArc(ezx.display, win, ezx.gc, x1 - ezx.thick / 2, y1 - ezx.thick / 2,
...
void ez_draw_line(Ez_window win, int x1, int y1, int x2, int y2)
{
    ...
    if (ezx.thick == 1)
        LineTo(ezx.hdc, x2 + 1, y2); /* final point excluded */
#endif /* EZ_BASE_ */
}
...
void ez_fill_rectangle(Ez_window win, int x1, int y1, int x2, int y2)
{
    ...
    if (ezx.thick != old_thick)
        ez_set_thick(old_thick);
#endif /* EZ_BASE_ */
}
...
void ez_draw_text(Ez_window win, Ez_Align align, int x1, int y1, char *STRING, int STRLength)
{
    ...
    //     if (buf[i] == '\n' || buf[i] == 0)
    //     {
    //         // x = x1 - XTextWidth(font, buf + j, i - j);
    //         // y = y1 + a + c * k - (c * n - b);
    //         if (fillbg == 0)
    //             XDrawString(ezx.display, win, ezx.gc, x, y, buf + j, i - j);
    //         else
    //             XDrawImageString(ezx.display, win, ezx.gc, x, y, buf + j, i - j);
    //         k++;
    //         j = i + 1;
    //         if (buf[i] == 0)
    //             break;
    ...
    if (fillbg == 0)
        XDrawString(ezx.display, win, ezx.gc, x1, y1, STRING, STRLength);
    else
        XDrawImageString(ezx.display, win, ezx.gc, x1, y1, STRING, STRLength);

#elif defined EZ_BASE_WIN32

    ez_cur_win(win);
    GetTextMetrics(ezx.hdc, &text_metric);
    // a = text_metric.tmAscent;
    // b = text_metric.tmDescent;
    // c = a + b;

    if (fillbg == 0)
        SetBkMode(ezx.hdc, TRANSPARENT);

    // /* Display line by line */
    // for (i = j = k = 0;; i++)
    //     if (buf[i] == '\n' || buf[i] == 0)
    //     {
    //         x = x1 - (i - j) * text_metric.tmAveCharWidth;
    //         y = y1 + c * k - (c * n - b) - 2;
    ...
    TextOut(ezx.hdc, x1, y1, (LPCSTR)STRING, STRLength);

    /* Restore background drawing mode */
    if (fillbg == 0)
        SetBkMode(ezx.hdc, OPAQUE);
...
int ez_draw_debug(void)
{
    static int debug = -1;
    if (debug < 0)
        debug = getenv("EZ_DRAW_DEBUG") != NULL;
    return debug;
}
...
void ez_event_dispatch(Ez_event *ev)
{
    ...
    if (ev->type == EzLastEvent)
        ...
        case Expose:
            ...
            if (ezx.dbuf_pix != None)
                ez_dbuf_preswap(ev->win);
...
LRESULT CALLBACK ez_win_proc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    ev.xev.hwnd = hwnd;
    ev.xev.message = msg;
    ev.xev.wParam = wParam;
    ev.xev.lParam = lParam;
    ...
    case WM_RBUTTONDOWN:
        ...
        ev.mb = msg == WM_LBUTTONDOWN ? 1 : msg == WM_MBUTTONDOWN ? 2
                                        : msg == WM_RBUTTONDOWN ? 3
                                                                : 4;
        /* If another button is pressed, it is ignored. */
        if (ezx.mouse_b != 0)
            return 0L;
    ...
    case WM_RBUTTONUP:
        ...
        ev.mx = GET_X_LPARAM(lParam);
        ev.my = GET_Y_LPARAM(lParam);
        ev.mb = msg == WM_LBUTTONUP ? 1 : msg == WM_MBUTTONUP ? 2
                                      : msg == WM_RBUTTONUP ? 3
                                                            : 4;
        /* If another button is pressed, it is ignored. */
        if (ezx.mouse_b != ev.mb)
...
{
    if (ez_prop_get(win, ezx.info_prop, (void **)info) < 0)
        return -1;
    if (*info == NULL)
    {
...
void ez_font_delete(void)
{
    ...
    for (i = 0; i < EZ_FONT_MAX; i++)
        if (ezx.font[i] != NULL)
        {
            XFreeFont(ezx.display, ezx.font[i]);
            ezx.font[i] = NULL;
        }
#elif defined EZ_BASE_WIN32
    int i;
...
int ez_color_init(void)
{
    ...
    case TrueColor:
        ...
        break;
    default:
        ez_error("ez_init_color: unsupported Visual\n");
        return -1;
    }
                    ...
                    int max, int inCube)
{
    ...
    if (inCube)
        ezx.pseudoColor.palette[R][G][B] = c.pixel;
...
// // #if 0
// #include <string>
// string ez_win_msg_name (unsigned int m)
// {
//     switch (m) {
